...
if ($result) {
    while ($row = $result->fetch_assoc()) {
        ...
        $is_risky = in_array($cat_id, [1, 2, 4]); // Fresh, Ikan, Seafood
        
        $status_label = 'Unknown';
        $status_class = 'bg-slate-100 text-slate-800';
        
        if ($is_risky) {
            // High Waste Risk Logic
            if ($pct < 50) {
                $status_label = 'High Waste Risk';
...
<body class="bg-background-light dark:bg-background-dark text-slate-600 dark:text-slate-300 font-display transition-colors duration-200 antialiased overflow-hidden h-screen flex">
    ...
    <main class="flex-1 flex flex-col h-full relative overflow-hidden">
        <?php $page_title = "Laporan Stok & Realisasi"; include ROOT_PATH . "includes/admin/header.php"; ?>
            ...
            <div class="max-w-7xl mx-auto">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                        ...
                        <p class="text-slate-500 dark:text-slate-400 mt-1">Ringkasan nilai aset stok dan realisasi penjualan harian.</p>
                    </div>
                    <div class="flex items-center gap-3">
                        ...
                        <button class="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors shadow-sm flex items-center gap-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    ...
                    <div class="bg-green-700 text-white p-6 rounded-xl border border-green-800 shadow-sm relative overflow-hidden">
                        <div class="relative z-10">
                            <p class="text-green-100 text-sm font-medium">Total Nilai Stok (Target)</p>
                            <h3 class="text-2xl font-bold mt-2">Rp <?php echo number_format($summary['total_stock_value_target'], 0, ',', '.'); ?></h3>
                            <div class="mt-4 inline-flex items-center px-2.5 py-0.5 rounded-full bg-green-800 bg-opacity-50 text-xs text-green-50 border border-green-600">
                                Estimasi Aset
                            </div>
                        </div>
                        <div class="absolute right-0 top-0 h-full w-24 bg-gradient-to-l from-green-600 to-transparent opacity-20"></div>
                        <span class="material-icons-round absolute right-4 bottom-4 text-6xl text-green-600 opacity-50">account_balance_wallet</span>
                    </div>

                    <!-- Card 2: Realization Value -->
                    <div class="bg-surface-light dark:bg-surface-dark p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden group">
                        ...
                        <div class="mt-2 flex items-baseline gap-2">
                            <span class="text-2xl font-bold text-slate-900 dark:text-white">Rp <?php echo number_format($summary['total_realized_value'], 0, ',', '.'); ?></span>
                        </div>
                    ...
                    <div class="bg-surface-light dark:bg-surface-dark p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden group">
                        ...
                        <div class="mt-2 flex items-baseline gap-2">
                             <span class="text-2xl font-bold text-slate-900 dark:text-white"><?php echo number_format($summary['total_target_weight'], 1, ',', '.'); ?></span>
                             <span class="text-sm text-slate-500 font-medium">kg</span>
                        </div>
                         <div class="mt-4 flex items-center text-sm">
                            <span class="text-slate-500 dark:text-slate-400">
                                Terjual: <span class="text-slate-900 dark:text-white font-semibold"><?php echo number_format($summary['total_realized_weight'], 1, ',', '.'); ?> kg</span>
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="bg-surface-light dark:bg-surface-dark p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden group">
                         <div class="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                            <span class="material-icons-round text-6xl text-amber-500">pie_chart</span>
                        </div>
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400">Efisiensi Stok</p>
                        <div class="mt-2 flex items-baseline gap-2">
                            <span class="text-2xl font-bold text-slate-900 dark:text-white"><?php echo number_format($efficiency, 1); ?>%</span>
                        </div>
                        <div class="mt-4 flex items-center text-sm">
                             <?php if($efficiency > 90): ?>
                                <span class="text-green-500 font-medium flex items-center">
                                     <span class="material-icons-round text-sm mr-1">check_circle</span> Optimal
                                </span>
                            <?php elseif($efficiency > 50): ?>
                                <span class="text-amber-500 font-medium flex items-center">
                                     <span class="material-icons-round text-sm mr-1">warning</span> Monitor
                                </span>
                            <?php else: ?>
                                <span class="text-red-500 font-medium flex items-center">
                                     <span class="material-icons-round text-sm mr-1">error</span> Low Efficiency
                                </span>
                            <?php endif; ?>
                        </div>
                    </div>
                                ...
                                <?php if(empty($products_data)): ?>
                                    <tr>
                                        <td colspan="7" class="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                                            <span class="material-icons-round text-4xl text-slate-300 mb-2">assignment_late</span>
                                ...
                                <?php else: ?>
                                    ...
                                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                            ...
                                            <div class="flex items-center gap-3">
                                                <div class="size-10 rounded-lg bg-slate-100 dark:bg-slate-700 shrink-0 overflow-hidden">
                                                    <?php if($item['product_image']): ?>
                                                        <img class="w-full h-full object-cover" src="../<?php echo $item['product_image']; ?>" alt="">
                                                    <?php else: ?>
                                                         <span class="material-icons-round text-slate-400 text-lg w-full h-full flex items-center justify-center">image</span>
                                                    <?php endif; ?>
                                                </div>
                                                <div>
                                                    <div class="font-medium text-slate-900 dark:text-white"><?php echo htmlspecialchars($item['product_name']); ?></div>
                                                    <div class="text-xs text-slate-500">ID: <?php echo $item['product_id']; ?></div>
                                                </div>
                                            </div>
                                        </td>
                                        <!-- Category -->
                                        ...
                                        <td class="px-6 py-4 align-middle">
                                            <div class="w-full max-w-xs">
                                                <div class="flex items-center justify-between mb-1">
                                                    <span class="text-sm font-bold text-slate-900 dark:text-white"><?php echo number_format($item['realized_qty'], 1, ',', '.'); ?> kg</span>
                                                    <span class="text-xs font-semibold <?php echo ($item['percentage'] >= 100) ? 'text-green-600' : 'text-slate-500'; ?>">
                                                        <?php echo number_format($item['percentage'], 1); ?>%
                                                    </span>
                                                </div>
                                                <div class="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2">
                                                    <div class="<?php 
                                                        if($item['percentage'] >= 100) echo 'bg-green-500'; 
                                                        elseif($item['percentage'] >= 50) echo 'bg-amber-500'; 
                                                        else echo 'bg-red-500';
                                                    ?> h-2 rounded-full transition-all duration-500" style="width: <?php echo min($item['percentage'], 100); ?>%"></div>
                                                </div>
                                            </div>
                                        </td>
                                        <!-- Gap -->
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium <?php echo $item['status_class']; ?> dark:bg-opacity-20">
